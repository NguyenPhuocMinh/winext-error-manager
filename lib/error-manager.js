...
function ErrorManager(params = {}) {
  ...
  this.errorBuilder = function (message) {
    ...
    loggerTracer.info(`Load function errorBuilder by ${name}-${version} successfully!`);
    try {
      const error = {};
  ...
  this.requiredModule = function (errorMessage, moduleName) {
    try {
      ...
      loggerTracer.info(`Load function requiredModule by ${name}-${version} successfully!`);
  ...
  this.invalidModule = function (errorMessage) {
    try {
      ...
      loggerTracer.info(`Load function invalidModule by ${name}-${version} successfully!`);
  ...
  this.configNotFound = function (errorMessage) {
    try {
      ...
      loggerTracer.info(`Load function configNotFound by ${name}-${version} successfully!`);
